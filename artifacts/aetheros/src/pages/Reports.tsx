...
export default function ReportsPage() {
  ...
  const energyMix = [
    ...
    { name: "Biomass", v: 11, color: "#3B82F6" },
  ...
  return (
    <Layout>
      <PageHeader
        ...
        right={
          <button className="flex items-center gap-2 px-3 py-2 rounded-md bg-[#2563EB] hover:bg-[#1D4ED8] text-[#F1F5F9] text-[12px] font-display font-semibold" data-testid="button-generate-report">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <Kpi label="YTD Energy Savings" value={fmtR(ytdSavings)} delta="+18% YoY" icon={<TrendingUp size={16} className="text-[#0EA5E9]" />} accent="green" />
        <Kpi label="YTD CO₂ Avoided" value={`${ytdCo2.toFixed(1)} t`} delta="+22% YoY" icon={<TrendingUp size={16} className="text-[#0EA5E9]" />} accent="green" />
        <Kpi label="Avg PUE" value={metrics.pue.toFixed(2)} delta="−0.21 vs SA avg" icon={<TrendingUp size={16} className="text-[#2563EB]" />} accent="teal" />
        <Kpi label="Uptime SLA" value="99.94%" delta="+0.04 vs target" icon={<TrendingUp size={16} className="text-[#0EA5E9]" />} accent="green" />
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        {/* Savings trend */}
        <div className="card-surface p-4 xl:col-span-2">
          <h3 className="font-display font-semibold text-[#0F172A] text-sm mb-3">Monthly Savings Trend</h3>
          <div className="h-[260px] bg-[rgba(241,245,249,0.7)] rounded-lg p-3">
        ...
        <div className="card-surface p-4">
          <h3 className="font-display font-semibold text-[#0F172A] text-sm mb-3">Energy Mix · YTD</h3>
          <div className="h-[230px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={energyMix} dataKey="v" nameKey="name" innerRadius={50} outerRadius={80} stroke="#F1F5F9" strokeWidth={2}>
          ...
          <div className="text-[11px] text-[#64748B] text-center mt-1">94% renewable + storage</div>
        </div>
      </div>
      ...
      <div className="card-surface mt-5 overflow-hidden">
        <div className="px-4 py-3 border-b border-[rgba(37,99,235,0.1)] flex items-center justify-between">
          <h3 className="font-display font-semibold text-[#0F172A] text-sm">Generated Reports</h3>
          <span className="text-[10px] font-mono-num text-[#64748B] uppercase tracking-wider">Auto-archived · S3 cold storage</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-[12px]">
            ...
            <tbody className="font-mono-num">
              {reports.map((r) => (
                <tr key={r.name} className="border-b border-[rgba(37,99,235,0.04)] last:border-0 hover:bg-[rgba(37,99,235,0.03)]">
                  <td className="px-4 py-3 text-[#0F172A]">{r.name}</td>
                  <td className="px-4 py-3 text-[#64748B]">{r.date}</td>
                  <td className="px-4 py-3 text-[#64748B]">{r.size}</td>
                  <td className="px-4 py-3"><span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-[rgba(37,99,235,0.1)] text-[#2563EB] border border-[#2563EB]/30">{r.type}</span></td>
                  <td className="px-4 py-3 text-right">
                    <button className="text-[#2563EB] hover:text-[#1D4ED8] text-[11px] flex items-center gap-1 ml-auto" data-testid={`button-download-${r.name}`}>
...
function Kpi({ label, value, delta, icon, accent }: { label: string; value: string; delta: string; icon: React.ReactNode; accent: "green" | "teal" }) {
  const c = accent === "green" ? "text-[#0EA5E9]" : "text-[#2563EB]";
  return (
    <div className="card-surface px-4 py-3">
      ...
      <div className="font-display font-bold font-mono-num text-[22px] text-[#0F172A] leading-tight">{value}</div>
